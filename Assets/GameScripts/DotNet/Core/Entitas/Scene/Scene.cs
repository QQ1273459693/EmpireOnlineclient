...
namespace TEngine
{
    public class Scene : Entity, INotSupportedPool
    {
        ...
        /// <returns></returns>
        public static async FTask<Scene> Create(Server server, int sceneType = 0, int sceneSubType = 0, long sceneId = 0, uint worldId = 1, string networkProtocol = null, string outerBindIp = null, int outerPort = 0)
        {
            if (sceneId == 0)
            {
