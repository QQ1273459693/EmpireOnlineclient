...
public partial class TranslucentImageBlurRenderPass : ScriptableRenderPass
{
    ...
    readonly struct BlurExecutionData
    {
    ...
    readonly struct PreviewExecutionData
    {
        public readonly TranslucentImageSource blurSource;
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        if (currentPassData.shouldUpdateBlur)
        {
            ...
            for (int i = 0; i < scratchesCount; i++)
            {
                ...
                cmd.GetTemporaryRT(ShaderId.intermediateRT[i], desc);
                blurAlgorithm.SetScratch(i, ShaderId.intermediateRT[i]);
            }

            {
                var blurExecData = new BlurExecutionData(sourceTex,
                                                         blurSource,
                                                         blurAlgorithm);
                ExecuteBlur(cmd, ref blurExecData);
            }
    ...
    static void ExecuteBlur(CommandBuffer cmd, ref BlurExecutionData data)
    {
    ...
    static void ExecutePreview(CommandBuffer cmd, ref PreviewExecutionData data)
    {
        var blurSource = data.blurSource;
