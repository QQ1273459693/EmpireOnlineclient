        ...
        if (currentPassData.shouldUpdateBlur)
        {
            ...
            using (var builder = renderGraph.AddUnsafePass<BlurRGPassData>(PROFILER_TAG, out var data))
            {
                ...
                for (int i = 0; i < scratchesCount; i++)
                {
                    ...
                    data.scratches[i] = UniversalRenderer.CreateRenderGraphTexture(renderGraph, desc, scratchNames[i], false, FilterMode.Bilinear, TextureWrapMode.Clamp);
                ...
                builder.SetRenderFunc(static (BlurRGPassData data, UnsafeGraphContext context) =>
                {
                    ...
                    var blurExecData = new BlurExecutionData(
                        data.sourceTex,
                        data.blurSource,
                        data.blurAlgorithm
                    );
                    ExecuteBlur(CommandBufferHelpers.GetNativeCommandBuffer(context.cmd), ref blurExecData);
                });
            }
        }
