...
namespace Water2D
{
    ...
    public class LayerRenderer  
    {
        ...
        [HideInInspector] [SerializeField] [Range(1f,2f)] protected float scale = 1f;
        ...
        private void CreateRTSpriteRenderer(SpriteRenderer sr, Camera mCamera)
        {
            ...
            if (layerTexture != null) layerTexture.Release();
            if (layerTexture == null) layerTexture = new RenderTexture((int)(mCamera.scaledPixelWidth * res * perX * Mathf.Max(1f, scale)), (int)(mCamera.scaledPixelHeight * res * perY * Mathf.Max(1f, scale)), 0, format, 0);
            else
            {
                layerTexture.width = (int)(mCamera.scaledPixelWidth * res * perX * Mathf.Max(1f, scale) );
                layerTexture.height = (int)(mCamera.scaledPixelHeight * res * perY * Mathf.Max(1f, scale));
            }
        ...
        private void CreateRTCamera(Camera mCamera)
        {
            ...
            if (layerTexture != null) layerTexture.Release();
            if (layerTexture == null) layerTexture = new RenderTexture((int)((float)mCamera.scaledPixelWidth * res * Mathf.Max(1f, scale)), (int)((float)mCamera.scaledPixelHeight * res * Mathf.Max(1f, scale)), 0, format, 0);
            else
            {
                layerTexture.width = (int)((float)mCamera.scaledPixelWidth * res * Mathf.Max(1f, scale));
                layerTexture.height = (int)((float)mCamera.scaledPixelHeight * res * Mathf.Max(1f, scale));
            }
        ...
        /// </summary>
        public void Setup(Camera mCamera, Transform holder, string layerName, float resolution = 1,  RenderTextureFormat format = RenderTextureFormat.ARGB32, float scale = 1, FilterMode filterMode = FilterMode.Point, float bitdepth = 0)
        {
            ...
            mainCamera.orthographicSize = mCamera.orthographicSize * this.scale;
