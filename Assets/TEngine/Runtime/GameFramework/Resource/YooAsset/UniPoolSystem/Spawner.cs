...
namespace YooAsset
{
	public class Spawner
	{
		...
		private readonly Dictionary<string,GameObject> _poolRootTrans=new Dictionary<string, GameObject>(100);
		private readonly GameObject _spawnerRoot;
		private readonly GameObject _poolingRoot;
		...
		private CreatePoolOperation CreateGameObjectPoolInternal(string location, bool dontDestroy = false, int initCapacity = 0, int maxCapacity = int.MaxValue, float destroyTime = -1f)
		{
			...
			{
				GameObject spawnerRoot = new GameObject(location + "Pool");
				spawnerRoot.transform.SetParent(_poolingRoot.transform);
				_poolRootTrans.Add(location+"Pool", spawnerRoot);
